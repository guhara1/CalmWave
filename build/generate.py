...
# 모든 페이지 공통 가격표 (메인과 동일 요금). 본문(</main>) 안 맨 끝에 들어감.
PRICING = """
    <section class="section" id="course" style="padding-top:0;">
      <div class="container">
        <div class="section-head"><h2>이용 코스와 요금</h2>
          <p>60·90·120분 코스별 기준 요금이며, 추가 비용 없이 안내드립니다.</p></div>
        <div class="pricing">
          <article class="plan">
            <div class="plan__name">60분 코스</div>
            <div class="plan__price">90,000<small>원</small></div>
            <div class="plan__dur">60분</div>
            <p class="plan__desc">기본 컨디션·릴랙스 케어</p>
            <a class="btn btn--ghost btn--block" href="tel:0508-202-4719">예약 문의</a>
          </article>
          <article class="plan plan--featured">
            <span class="plan__badge">추천</span>
            <div class="plan__name">90분 코스</div>
            <div class="plan__price">150,000<small>원</small></div>
            <div class="plan__dur">90분</div>
            <p class="plan__desc">아로마 포함 추천 구성</p>
            <a class="btn btn--accent btn--block" href="tel:0508-202-4719">예약 문의</a>
          </article>
          <article class="plan">
            <div class="plan__name">120분 코스</div>
            <div class="plan__price">180,000<small>원</small></div>
            <div class="plan__dur">120분</div>
            <p class="plan__desc">전신 집중 프리미엄 케어</p>
            <a class="btn btn--ghost btn--block" href="tel:0508-202-4719">예약 문의</a>
          </article>
        </div>
        <p class="pricing-note">지역·예약 시간대·이동 거리에 따라 상담 시 최종 확인됩니다.
          <a href="/incheon-bucheon-siheung/check/travel-fee.html">상세 요금 안내 보기 →</a></p>
      </div>
    </section>"""
...
AREAS = [
 {"slug":"songdo-yeonsu-nonhyeon","name":"송도·연수·논현권",
  ...
  "h1":"송도·연수·논현 출장마사지 · 국제업무지구·호텔 방문 안내",
 ...
 {"slug":"guwol-namdong-bupyeong","name":"구월·남동·부평권",
  ...
  "h1":"구월·남동·부평 출장마사지 · 인천 내륙 핵심 상권 방문 안내",
 ...
 {"slug":"cheongna-seohae-geomdan","name":"청라·서해·검단권",
  ...
  "h1":"청라·서해·검단 출장마사지 · 신도시·오피스텔 방문 안내",
 ...
 {"slug":"yeongjong-airport-jemulpo","name":"영종·공항·제물포권",
  ...
  "h1":"영종·공항·제물포 출장마사지 · 공항·항만 숙소 방문 안내",
 ...
 {"slug":"bucheon-jungdong-sangdong-songnae","name":"부천 중동·상동·송내권",
  ...
  "h1":"부천 중동·상동·송내 출장마사지 · 핵심 상권 방문 안내",
 ...
 {"slug":"yeokgok-sosa-okgil","name":"부천 역곡·소사·옥길권",
  ...
  "h1":"부천 역곡·소사·옥길 출장마사지 · 서울 연결 주거 방문 안내",
 ...
 {"slug":"wonjong-gogang-ojeong","name":"부천 원종·고강·오정권",
  ...
  "h1":"부천 원종·고강·오정 출장마사지 · 북부 생활권 방문 안내",
  "incs":"원종, 고강, 오정, 성곡, 신흥",
 ...
 {"slug":"baegot-jeongwang-oido","name":"시흥 배곧·정왕·오이도권",
  ...
  "h1":"시흥 배곧·정왕·오이도 출장마사지 · 서부 핵심 생활권 방문 안내",
 ...
 {"slug":"eungye-janghyeon-mokgam","name":"시흥 은계·장현·목감권",
  ...
  "h1":"시흥 은계·장현·목감 출장마사지 · 동부 주거 생활권 방문 안내",
...
def render_district(city_key, dist):
    ...
    reps = [d[1] for d in dist["dongs"][:3]]
    if reps:
        dist_h1 = f"{dist['name']} 출장마사지 · " + "·".join(reps) + " 방문 안내"
        title = f"{dist['name']} 출장마사지｜" + "·".join(reps[:2]) + " 홈타이 방문 안내 — 간다GO"
    else:
        dist_h1 = f"{dist['name']} 출장마사지 · 방문 홈타이 이용 안내"
        title = f"{dist['name']} 출장마사지｜방문 홈타이 이용 안내 — 간다GO"
    mdesc = f"{dist['name']} 출장마사지·홈타이 방문 이용 기준과 주요 생활권을 안내합니다."
    faqs = [
    ...
    s += f"""
  <main id="main">
    {page_hero(f"{city['name']} {city['mid_label']}", dist_h1)}{body}
    <section class="section" style="padding-top:0;">
      <div class="container narrow"><div class="card"><h2>예약 전 체크리스트</h2>{CHECKLIST}</div></div>
    </section>
    <section class="section" style="padding-top:0;"><div class="container narrow">{NOTICE}</div></section>
...
{FOOT}"""
    ...
    for slug, name, lines, desc, rel in STATIONS:
        ...
        title = f"{name} 출장마사지｜역세권 자택·숙소 방문 홈타이 — 간다GO"
        ...
        st += f"""
  <main id="main">
    {page_hero(f"지하철 · {lines}", f"{name} 출장마사지 · 역세권 자택·숙소 방문 케어")}
    <section class="section">
      <div class="container prose">
